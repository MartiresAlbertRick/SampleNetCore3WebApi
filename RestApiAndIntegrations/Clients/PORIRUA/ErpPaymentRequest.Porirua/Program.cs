    ...
    internal class Program
    {
        ...
        private static async Task<ExitCode> ExportPaymentRequests(ProgramOptions options, string[] args)
        {
            try
            {
                ...
                var paymentRequest = new PaymentRequest(new DBConfiguration { ConnectionString = ConnectionString, DateFormat = AppSettings.DateFormat }, AppSettings);
                await paymentRequest.Export().ConfigureAwait(false);
            }
            catch (Exception e)
            {
